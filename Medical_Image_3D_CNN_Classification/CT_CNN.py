...
import tensorflow as tf
from CT_3d_CNN_Classification import *
import dcmstack
...
class Hemorrhage_Classification():
    ...
    def __init__(self, param):
        ...
        self.x = tf.placeholder(tf.float32, shape=[None, self.width*self.height*self.depth]) # [None, 28*28]
        self.y_ = tf.placeholder(tf.float32, shape=[None, self.nLabel])  # [None, 10]
    ...
    def classification(self):
        ...
        x_image = tf.reshape(self.x, [-1,self.width,self.height,self.depth,1]) # [-1,28,28,1]
        ...
        ## Readout Layer
        W_fc2 = weight_variable([1024, self.nLabel]) # [1024, 10]
        b_fc2 = bias_variable([self.nLabel]) # [10]
        ...
        for i in range(100):
            batch = get_data_CT(self.sess,'Train',20)
            # Logging every 100th iteration in the training process.
            if i%5 == 0:
                train_accuracy = accuracy.eval(feed_dict={self.x:batch[0], self.y_: batch[1], keep_prob: 1.0})
                print("step %d, training accuracy %g"%(i, train_accuracy))
            train_step.run(feed_dict={self.x: batch[0], self.y_: batch[1], keep_prob: 0.5})

        # Evaulate our accuracy on the test data
        testset = get_data_CT(self.sess,'Test',30)
        print("test accuracy %g"%accuracy.eval(feed_dict={self.x: testset[0], self.y_: testset[1], keep_prob: 1.0}))
